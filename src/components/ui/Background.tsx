...
// Web版の body 背景（斜めのグラデーション＋3点のにじみ）をそのまま持ってきた壁紙。
// ノート式を開いているあいだは、紙に集中できるよう無地のグレーに切り替える。
export function Background({ chic: chicMode }: { chic: boolean }) {
  const { width, height } = useWindowDimensions();
  const base = useSvgId("bgBase");
  const lav = useSvgId("bgLav");
  const mint = useSvgId("bgMint");
  const rose = useSvgId("bgRose");

  if (chicMode) {
    return <View style={[StyleSheet.absoluteFill, { backgroundColor: chic.bg }]} />;
  }
